...
def train_ppo(config=None):
    ...
    log_every = cfg["PPO_LOG_EVERY"]
    save_every = cfg["PPO_SAVE_EVERY"]
    start = time.time()
    ...
    while iters < cfg['PPO_TOTAL_ITERS']:
        ...
        with torch.inference_mode():
            ...
            for step in rollout_bar:
                ...
                kl_rewards.append(rewards_T[:, step].mean().item())
        ...
        avg_kl = sum(kl_rewards) / max(len(kl_rewards), 1)
        ...
        ppo_model.train()
        update_losses = []
        for _ in tqdm(range(0, cfg["PPO_EPOCHS"] * NT, cfg["PPO_MINIBATCH"]), desc="ppo update", leave=False):
            ...
            loss = train_step(ppo_model, optimizer, input_ids_mb, attention_mask_mb, actions_T[n_idx, t_idx], gae[t_idx, n_idx], returns[t_idx, n_idx], log_probs_T[n_idx, t_idx], clip_eps, cfg["PPO_C1"], pretrain_input_ids=pretrain_input_ids, pretrain_attention_mask=pretrain_attention_mask, pretrain_gamma=pretrain_gamma)
            update_losses.append(loss.item())

        rollout += 1
        if rollout % save_every == 0:
        ...
        if wandb_run is not None and rollout % log_every == 0:
            elapsed = time.time() - start
            wandb.log(
                {
                    "rollout/avg_reward": avg_reward,
                    "rollout/avg_rm": avg_rm,
                    "rollout/avg_kl": avg_kl,
                    "train/loss": sum(update_losses) / max(len(update_losses), 1),
                    "train/update_steps": update_steps,
                    "perf/sps": iters / max(elapsed, 1e-6),
                    "global_step": iters,
                },
                step=rollout,
            )
    ...
    if wandb_run is not None:
        wandb.log({"train/total_time_s": time.time() - start}, step=rollout)
        wandb.finish()
